...
| harmony-core unit tests | `packages/harmony-core/tests/{music-theory,candidates,scoring,styles,midi-export,midi-import}.test.ts` | 50 | chord-tone construction, scale membership, range clamping, each of the 13 scoring components individually (including leap penalty, dissonance penalty, tension resolution, common-tone handling, parallel-fifths penalty, and `voiceLeading`'s two-sided backward+forward blend), style differentiation, MIDI byte structure (export and a round-trip import) |
...
| harmony-core section regeneration | `packages/harmony-core/tests/regenerate-section.test.ts` | 7 | notes outside the target section stay byte-identical, full note coverage, determinism, unknown-section-id error, cross-section isolation (regenerating the verse doesn't touch the chorus), vocal-range continuity into the regenerated section, and a two-sided-continuity test that forces two different pitches onto the locked note right after a regenerated section and confirms the section's last note responds to each (proving the forward seam is genuinely read, not coincidental) |
...
| Web unit tests | `apps/web/tests/unit/{storage,project-store,EditorPage,LandingPage,Root,piano-roll-geometry,audio-engine,recorder}.test.tsx` | 83 | multi-project IndexedDB storage (via `fake-indexeddb`: list/load/save/delete by id, last-opened tracking, the legacy single-slot migration, skipping corrupted entries), the Zustand store's mutating actions, generation flow, section-regenerate guard/error path, and multi-project actions (fork-on-sample-load, keep-id-on-import, open/delete/refresh), an integration test that drives the actual table UI to add a note/chord and generate a real arrangement, landing page content, hash-based view switching, the pure pixel↔beat/pitch conversion + drag-to-patch math behind piano-roll editing (both `dragToNotePatch` for melody notes and `dragToBandPatch` for chord/section bands), guide-playback scheduling (frequency/timing/gain math, region-slicing for A-B loop, and count-in click timing/accent, all verified against a fake `AudioContext`), and recording chunk-assembly (verified against a fake `MediaRecorder` + `MediaStream`, plus the no-`mediaDevices`-API error path via a stubbed `navigator`) |
| Web e2e | `apps/web/tests/e2e/{landing,editor,piano-roll-drag,piano-roll-band-drag,section-regenerate,playback,recording,sync-playback-recording,project-management}.spec.ts` | 33 | Playwright, real Chromium: landing page loads with no console errors; loading a sample project and generating actually produces a result; switching styles actually changes the per-note result table (not just the rounded score); no console errors while using the editor; dragging a note moves it (verified via the note table's actual values, not just visually); resizing via the drag handle changes duration; a plain click selects rather than moving; double-click adds a note; Delete removes the selected note; dragging a chord or section band moves its start time (verified via the chord/section table's actual values) and resizing via its right-edge handle changes duration/endTime, with no console errors and no regression to the pre-existing note-drag locators; the section-regenerate button's disabled/enabled state and that clicking it doesn't error; guide playback status text during play/stop/both-tracks and genuine auto-stop once a short note's real playback time elapses; A-B loop keeps replaying well past when a one-shot would have auto-stopped; count-in genuinely delays playback and still auto-stops afterward; recording produces a real, playable, downloadable clip against Chromium's fake media device; "재생하며 녹음" genuinely starts and stops both guide playback and microphone recording together, and falls back to melody-only playback when no harmony has been generated yet; editing a blank project actually saves it and lists it under "최근 프로젝트", "새 프로젝트" doesn't delete the previous one, opening a listed project switches the editor's actual fields, and deleting the currently-open project falls back to a fresh blank one — all against real IndexedDB autosave timing |

Total: 240 tests, all passing as of this writing. Run `pnpm test` for unit
...
- **Real-microphone recording quality** — e2e tests exercise
  `getUserMedia`/`MediaRecorder` for real, but against Chromium's
  synthetic fake device (`--use-fake-device-for-media-stream`), not an
  actual microphone; audio *quality* (as opposed to "does recording
  mechanically work") isn't something these tests can speak to.
